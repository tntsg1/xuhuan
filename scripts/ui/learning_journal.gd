...
func _build() -> void:
	...
	var title := _make_label(GameManager.text("📖  Club Logbook", "📖  俱乐部观测日志"), 24)
...
func _populate() -> void:
	...
	if entries.is_empty():
		var empty := _make_label(GameManager.text(
			"No observations recorded yet.\nComplete a mission to see your first entry here!",
			"还没有观测记录。\n完成一次观测来看到你的第一条记录！"
		), 16)
...
func _build_entry_card(num: int, name_en: String, name_zh: String, quality: String, assembly: float, learn_en: String, learn_zh: String) -> Control:
	...
	# Object name — use dict_text pattern with "both" fallback
	var name_label := _make_label(GameManager.text(name_en, name_zh), 19)
	...
	var badge := _make_label(quality_display, 12)
	...
	# Assembly score
	var asm_label := _make_label(GameManager.text("Assembly: %d", "组装: %d") % int(assembly), 13)
	...
	# Learning text — show only current language
	var learn_display := GameManager.text(learn_en, learn_zh)
	var learn_label := _make_label(learn_display, 14)
	...
	var lines := float(maxf(1.0, ceilf(float(learn_display.length()) / 80.0)))
