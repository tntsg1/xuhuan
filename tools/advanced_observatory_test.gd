...
func _initialize() -> void:
	...
	if game_manager == null:
		...
		quit(1)
		return
	var mission_manager: Node = root.get_node_or_null("/root/MissionManager")
	game_manager.new_game()
	_expect(game_manager.levels_data.size() == 85, "level data ends at FAST graduation L85")
	...
	_expect(mission_manager != null and mission_manager.get_level(86).is_empty(), "post-FAST L86 has been removed")
